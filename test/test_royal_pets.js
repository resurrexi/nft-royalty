...
contract("RoyalPets", function(accounts) {
  it("should support the ERC721 and ERC2198 standards", async () => {
    ...
    const isERC721 = await royalPetsInstance.supportsInterface(
      ERC721InterfaceId
    );
    const isERC2981 = await royalPetsInstance.supportsInterface(
      ERC2981InterfaceId
    );
  ...
  it("should return the correct royalty info when specified and burned", async () => {
    const royalPetsInstance = await RoyalPets.deployed();
    // 1st token uses default minting method with default royalty fee
    await royalPetsInstance.mintNFT(accounts[0], "fakeURI");
    // 2nd token uses minting that sends royalties to another account with custom royalty fee
    ...
    // get royalty info for 1st token
    const defaultRoyaltyInfo = await royalPetsInstance.royaltyInfo.call(
      1,
      1000
    );
    // get royalty info for 2nd token
    let tokenRoyaltyInfo = await royalPetsInstance.royaltyInfo.call(2, 1000);
    ...
    // Custom royalty percentage taken should be 10%.
